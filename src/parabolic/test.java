...
public class test
{
    ...
    static double calculate(String pattern)
    {
        ...
        while (i < s.length())
        {
            ...
            {
                case '+':
                    stack.add(0, Double.parseDouble(doubleString[k]));
                    k++;
                    break;
                case '-':
                    stack.add(0, -1 * Double.parseDouble(doubleString[k]));
                    k++;
                    break;
                case '*':
                    stack.add(0, Double.parseDouble(doubleString[k]) * stack.remove(0));
                    k++;
                    break;
                case '/':
                    stack.add(0, stack.remove(0) / Double.parseDouble(doubleString[k]));
                    k++;
                    break;
            }
            i++;
